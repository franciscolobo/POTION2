...
sub check_parameters { #check for all parameters, 
  ...
  if($parameters->{mode} =~ /branch/i){ # Parameter in branch-model
  ...
  if(defined $parameters->{infer_positive_selection}){
    ...
    if($parameters->{infer_positive_selection} eq "fastcodeml"){
  ...
  if (!defined $parameters->{infer_positive_selection} or !exists($parameters->{infer_positive_selection})) { die ("No program specified for infer positive selection, please set the parameter 'infer_positive_selection' in your project configuration file.\n"); }
  my @infer_positive_selection = ("codeml", "fastcodeml");
  $flag = 0;
  foreach my $program (@infer_positive_selection){
    if ($program eq "$parameters->{infer_positive_selection}") {$flag = 1;}
  }
  if ($flag == 0){
   die("Currently supported programs for infer positive selection are @infer_positive_selection. You chose $parameters->{infer_positive_selection}.\n"); 
  }
...
sub parse_homology_file {
  # LEO - Verificar a necessidade do sequences_ref. vira tudo pra sequence_data?
...
sub calculate_dn_ds {
  ...
  if (-s "$$ortholog_group.cluster.aa.fa.aln.nt.phy.trim.paml.model$model") { #for codeml
    ...
    # print "OPA antes do if\n";
    #       my $a=<STDIN>;
    # # Checking if the model isn't already processed and with complete information
    # if (-s "$$ortholog_group.cluster.fastcodeml.model$model") { #for fastcodeml
    #   #  open(my $fh_paml, "<", "$$ortholog_group.cluster.fastcodeml.model$model");
    #   my $file = "$$ortholog_group.cluster.fastcodeml.model$model";
    
    #   open (my $fh_paml, "<", $file);
    #   my $ln_value;
    #   while (my $line = <$fh_paml>) {
    #     chomp $line;
    #     next if ($line =~ /NA/);
    #     if ($line =~ /^(\s+)LnL/){
    #       $ln_value = find_ln_value_fast($line);
    #       print "OPA dentro do if! - $ln_value\n";
    #       my $a=<STDIN>;
    #     }
    #   }
    #   close ($fh_paml);
    #   #my $ln_value = find_ln_value_fast($fh_paml);
    #    # print "OPA! - $ln_value\n";
    #    # my $a=<STDIN>;
      
    #   my $BEB = exists_BEB($fh_paml) if (($model eq "2") || ($model eq "8"));
    # #    my $BEB = exists_BEB($fh_paml) if (($model == 2) || ($model == 8) || ($model eq "H1"));
    #   close ($fh_paml);
    #   if (defined $ln_value && (($model eq "1") || ($model eq "7") || ($model eq "8a") || ($BEB))) {
        
    # #    if (defined $ln_value && (($model == 1) || ($model == 7) || ($model eq "8a") || ($model eq "H0") || ($BEB))) {
    #     print LOG ("File $$ortholog_group.cluster.fastcodeml.model$model already exist and has all needed values, skipping the creation of DN/DS file for this model for group $$ortholog_group.\n");
    #     return;
    #   } elsif (!defined $ln_value) {
    #     print LOG ("File $$ortholog_group.cluster.fastcodeml.model$model already exist, but doesn't have the log-likelihood value of the model $model, calculating it for group $$ortholog_group.\n");
    #   } else {
    #     print LOG ("File $$ortholog_group.cluster.fastcodeml.model$model already exist, but doesn't have the information of the Bayes Empirical Bayes (BEB) analysis, calculating it for group $$ortholog_group.\n");
    #   }
    # }
  ...
  if ($parameters->{mode} eq "branch") {
    if($parameters->{infer_positive_selection} eq "codeml"){
      ...
      if (((-s "$$ortholog_group.cluster.aa.fa.aln.nt.phy.trim") && (-s "$$ortholog_group.dummy_tree_unrooted"))) {
        run_fastcodeml ($parameters, $ortholog_group, \$model);
        #print "Dentro\n";
        #my $a = <SDTIN>;
      }
      elsif (!-e "$$ortholog_group.cluster.aa.fa.aln.nt.phy.trim") {
...
sub create_paml_config_files {
  ...
  if ($parameters->{mode} eq "site") {
    ...
    print $fh_config_file ("seqfile = $$ortholog_group.cluster.aa.fa.aln.nt.phy.trim\n"); # .phy file used for sequence
    print $fh_config_file ("treefile = $$ortholog_group.dummy_tree_unrooted\n"); # .final_tree file with the consensus tree generated by phylip, uses the aminoacid file
    print $fh_config_file ("outfile = $$ortholog_group.cluster.aa.fa.aln.nt.phy.trim.paml.model$$model.i\n");
...
sub set_as_interleaved {
 ...
 # Function to set Phyllip Interleaved to sequential. Needed to run fastcodeml

sub set_as_sequential {
  my $phylip_file = shift;
  #my $ortholog_group = shift;
  
  my $file = Bio::AlignIO->new(
      -file => "$phylip_file",
      -format => "phylip"
    );
  #$file->interleaved(0);
   my $phylip_out = Bio::AlignIO->new(
       -format => 'phylip',
       #-fh     =>  \*STDOUT,#my $fh_sequential,
       -file => ">$phylip_file.seq",
       -interleaved => 0
     );
   while (my $aln = $file->next_aln()){
     $phylip_out->write_aln($aln);
   }
  # return $file;
}

sub find_ln_value_fast{
 my $file = shift;
  ...
  if ($file =~ /H0/){
    ...
    while (my $line = <$fh_paml>) {
      ...
      if ($line =~ /^(\s+)LnL0/){
        # print "$line\n";
        # my $a = <STDIN>;
        if ( $line =~ /(-*\d+\.\d+)/ ){ #example of $line: \s+LnL0:  -2131.973936702549963
          my $test = $1;
          print LOG ("PARSED lnL value: $test\n");
           # print ("PARSED lnL value: $test\n");
          return $test;
        }
        #my $test = $1 if ( $line =~ /(-*\d+\.\d+)/ ); #example of $line: \s+LnL0:  -2131.973936702549963
        # print LOG ("PARSED lnL value: $test\n");
        # print ("PARSED lnL value: $test\n");
        # return $test;
      }
...
sub parse_fastcodeml_result{
  ...
  my $q_values;
  my $p_value;
  my $fileH0 = "$$ortholog_group.cluster.fastcodeml.modelH0";
  my $fileH1 = "$$ortholog_group.cluster.fastcodeml.modelH1";

  # print "Model: $$model\n";
  # my $a = <STDIN>;
  if(-s $fileH0 && -s $fileH1){
    $H0_lnl = find_ln_value_fast($fileH0);
    $H1_lnl = find_ln_value_fast($fileH1);
  }
  if ($H0_lnl && $H1_lnl) {
    my $diff = 2*($H1_lnl - $H0_lnl);
    $p_value = Statistics::Distributions::chisqrprob(1, $diff);
    $data{$$ortholog_group} = $p_value;
    # print "$$ortholog_group\t$p_value\n";
    # my $a = <STDIN>;
  }
  my $p_values = \%data;
  #my $qvalues = BY($pvalues);
  $q_values = BY($p_values);
  my %hash = %{$q_values};
  # return ($p_value,%hash);

  foreach my $key(keys %hash) {
      # print("$key\t$hash{$key}\n");
      return ($p_value,$hash{$key});
  }
...
sub run_fastcodeml {
  ...
  my $tries = 0;
  my @run_model = split(//,$$model);
  
  # Fix Phy to run in fastcodeml
  my $phy_file = "./$$ortholog_group.cluster.aa.fa.aln.nt.phy.trim";
  if (!-s "$$ortholog_group.cluster.aa.fa.aln.nt.phy.trim.seq"){
    set_as_sequential($phy_file);
  }
  my $phy_seq = "./$$ortholog_group.cluster.aa.fa.aln.nt.phy.trim.seq";

  print ("Calculating linkelihood of model $$model for group $$ortholog_group\n");
  # print LOG ("$parameters->{fastcodeml_path} -v 0 -ou $$ortholog_group.cluster.fastcodeml.model$$model -hy $$model -s $rand -nt 1 -bl $$ortholog_group.dummy_tree_unrooted $phy_seq");
  print LOG ("$parameters->{fastcodeml_path} -v 0 -m 22 -ou $$ortholog_group.cluster.fastcodeml.model$$model -hy $run_model[1] -nt 1 -bl $$ortholog_group.dummy_tree_unrooted $phy_seq\n");

  while($tries < $parameters->{tries} && !-s "$$ortholog_group.cluster.fastcodeml.model$$model"){
    $tries++;
    try{
        # my $stderr = capture_stderr{ system ("$parameters->{fastcodeml_path} -v 0 -ou $$ortholog_group.cluster.fastcodeml.model$$model -hy $$model -s $rand -nt 1 -bl $$ortholog_group.dummy_tree_unrooted $phy_seq")};
        my $stderr = capture_stderr{ system ("$parameters->{fastcodeml_path} -v 0 -m 22 -ou $$ortholog_group.cluster.fastcodeml.model$$model -hy $run_model[1] -nt 1 $$ortholog_group.dummy_tree_unrooted $phy_seq")};
        
      }catch{
        if($tries >= $parameters->{tries} && !-s "$$ortholog_group.cluster.fastcodeml.model$$model" && defined $_) {
           print LOG_ERR ("Couldn't analyze the likelihood of model $$model for group $$ortholog_group.\nError: $_\n\n");
          die("Couldn't analyze the likelihood of model $$model for group $$ortholog_group.\nError: $_\n\n");
        }
      };
  }
  return;
...
sub construct_table_header_fast{
...
sub parse_dn_ds_results {
  ...
  # print Dumper(\%nested_models_to_be_tested);
  # my $a = <STDIN>;
  ...
  # if ($parameters->{mode} eq "branch") {
  #   my $header = construct_table_header_fast(\%nested_models_to_be_tested);
    ...
    if($parameters->{infer_positive_selection} eq "codeml"){
      ...
      if ($parameters->{mode} eq "branch"){
        ...
        if (-e "$ortholog_dir$ortholog_group.cluster.aa.fa.aln.nt.phy.trim.paml.modelH1") {
          ...
          # print("PUTZ\t$data{$ortholog_group}{gene_id}, $data{$ortholog_group}{temp_id}, $data{$ortholog_group}{selected_aminoacids_H1}\n\n");
          close($fh_paml);
        }  
      }
    }else{ # $parameters->{infer_positive_selection} eq 'fastcodeml'
      if ($parameters->{mode} eq "branch"){
        if (-e "$ortholog_dir$ortholog_group.cluster.fastcodeml.modelH0") {
          #open (my $fh_paml, "<", "$ortholog_dir$ortholog_group.cluster.fastcodeml.modelH0");
          my $file = "$ortholog_dir$ortholog_group.cluster.fastcodeml.modelH0";
          # open (my $fh_paml, "<", $file);
          #while (my $line = <$fh_paml>) {
          #  chomp $line;
          #  if ($line =~ /^(\s+)LnL0/){
          #    print "Linha $line\n";
              $data{$ortholog_group}{H0} = find_ln_value_fast($file);
          #    my $lnl = find_ln_value_fast($line);
          #    print "$lnl\n";
          #    my $a = <STDIN>;
              # print Dumper($data{ortholog_group}{H0});
          #  }
          #}
          # close ($fh_paml);
          # $data{$ortholog_group}{H0} = find_ln_value_fast($fh_paml);
          # close($fh_paml);
        }
        if (-e "$ortholog_dir$ortholog_group.cluster.fastcodeml.modelH1") {
          # open (my $fh_paml, "<", "$ortholog_dir$ortholog_group.cluster.fastcodeml.modelH1");
          my $file = "$ortholog_dir$ortholog_group.cluster.fastcodeml.modelH1";
          # open (my $fh_paml, "<", $file);
          #while (my $line = <$fh_paml>) {
          #  chomp $line;
          #  if ($line =~ /^(\s+)LnL0/){
              $data{$ortholog_group}{H1} = find_ln_value_fast($file);
          #  }
          #}
          # open (my $fh_paml, "<", $file);
           # $data{$ortholog_group}{H1} = find_ln_value_fast($file);
      ...
      }else{ die("$parameters->{infer_positive_selection} supports only branch model in POTION2"); }
    } # close else fatcodeml
...
#        print("OPAHereLALA!\n\n");
  ...
  #    my $a = <STDIN>;
    ...
    if (scalar keys %p_value_H0H1) {
      ...
      # foreach my $key (keys %p_value_H0H1) {
      #   print("LALALA\t$key\t$p_value_H0H1{$key}\n");
      # }
      if($@) { $q_values_H0H1 = BY(\%p_value_H0H1); }
      # my %lala = %{$q_values_H0H1};
      # foreach my $key (keys %lala) {
      #   print("OPAOPAOPA\t$key\t$lala{$key}\n\n");
      # } 
    }
  }
...
sub manage_task_allocation {
  ...
  #    if ($parameters->{recombination_qvalue} =~ /N.A./i) {
  #      push(@{$f_tree_to_process}, "$ortholog_group;f_tree");
  #      print ("uLALA\t@{$f_tree_to_process}\n");
  #      my $a = <STDIN>;
  #    }
...
sub trim_tree_file {
  ...
  #$stderr = capture_stderr {system ("$parameters->{nw_reroot} -d $$ortholog_group.dummy_tree > $$ortholog_group.dummy_tree_unrooted")};
  #if ($stderr) {move ("$$ortholog_group.dummy_tree", "$$ortholog_group.dummy_tree_unrooted")}; 
  system ("cp $$ortholog_group.dummy_tree $$ortholog_group.dummy_tree_unrooted");
  ...
  my $dummy_tree = Bio::TreeIO->new('-format' => 'newick', '-file' => "$$ortholog_group.dummy_tree_unrooted")->next_tree();
  ...
  if ($check_mark eq 'Not_ok'){
    ...
    $lca->id($lca->id."#1");
    Bio::TreeIO->new ('-format' => 'newick' , '-file' => ">$$ortholog_group.dummy_tree_unrooted")->write_tree($dummy_tree); 
  } 
  
  $stderr = capture_stderr {system ("$parameters->{nw_clade} $$ortholog_group.dummy_tree_unrooted '#1' > $$ortholog_group.tmp_foreground_branch")};
  $stderr = capture_stderr {system ("$parameters->{nw_labels} $$ortholog_group.tmp_foreground_branch > $$ortholog_group.foreground_branch")};
  my @foreground_names;
